...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        curr_x = self._start * np.ones((N, 1), dtype=np.int32)
        
        # loop for all sequences
        for t in range(max_length):  
            word_embed, _ = word_embedding_forward(curr_x, W_embed)
            if self.cell_type == 'rnn':
                ...
                prev_c = c # prepare for next step
            scores, _ = temporal_affine_forward(h[:, np.newaxis, :], W_vocab, b_vocab)
            max_idx = np.squeeze(np.argmax(scores, axis=2))
            captions[:, t] = max_idx
