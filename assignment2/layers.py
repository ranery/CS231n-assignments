...
function: layers forward and backward
...
def affine_backward(dout, cache):
    ...
    - db: Gradient with respect to b, of shape (M,)  (1,M)
    """
    x, w, b = cache
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    (x, sample_mean, sample_var, sample_std, x_normalized, beta, gamma, eps) = cache 
    N = x.shape[0] 
    dbeta = np.sum(dout, axis=0) 
      ...
      for c in range(C): 
        for i in range(H_out): 
          for j in range(W_out): 
            pool_i = i * stride 
            pool_j = j * stride 
            out[n, c, i, j] = np.max(x[n, c, pool_i : pool_i + HH, pool_j : pool_j + WW]) 
      ...
      for c in range(C): 
        for i in range(H_out): 
          for j in range(W_out): 
            pool_i = i * stride 
            pool_j = j * stride 
            pool_area = x[n, c, pool_i : pool_i + HH, pool_j : pool_j + WW] 
            pool_max = np.max(pool_area) 
            pool_max_mask = 1.0 * (pool_area == pool_max) 
            dx[n, c, pool_i : pool_i + HH, pool_j : pool_j + WW] += pool_max_mask * dout[n, c, i, j] 
